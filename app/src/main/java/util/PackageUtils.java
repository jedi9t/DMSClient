...
public class PackageUtils {
    ...
    public static final int install(Context context, String filePath,int type) {
        if (PackageUtils.isSystemApplication(context) || ShellUtils.checkRootPermission()) { 
        	Log.i(TAG, "���ھ�Ĭ��װ");
            return installSilent(context, filePath);
        } 
        Log.i(TAG, "��Ȩ�޾�Ĭ��װ");
        return installNormal(context, filePath,type) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;
    }
     ...
     */
    public static boolean installNormal(Context context, String filePath,int type) {
    ...
    public static int getInstallLocation() {
        ...
        if (commandResult.result == 0 && commandResult.successMsg != null && commandResult.successMsg.length() > 0) {
            try {
                ...
                Log.d(TAG, "pm get-install-location success");
